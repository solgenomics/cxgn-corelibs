#$Id: Central.pm,v 1.224 2005/12/09 16:49:54 mwilkinson Exp $
...
sub registerService {
	...
	if ( defined $signatureURL ) {
		my $ch = 0;
		my $i;
		foreach $i ( $serviceName, $serviceType, $AuthURI, $contactEmail, $URL,
			$desc, $Category )
		{
			if ( defined $i ) {
				$ch = 1;
		...
		if ( $ch == 0 ) {
			...
			return &_success( "The RDFagent call was successful.  Report will send to you by E-mail",
			  ...
			  if ( $rez == 0 );
			return &_error(
				"Some problem with a connection or RDF model building", "" )
			  if ( $rez != 0 );
		}
	}
