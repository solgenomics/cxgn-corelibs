...
##	Modified by C. Carpita here
	foreach my $class (@classes) {
		my $filepath = $self->_class_to_file($class);
		my $timestamp = _last_mod($filepath);
		chomp($timestamp);
		my $uri = $self->_class_to_uri($class);
		push(@uris, $uri . "?v=" . $timestamp);
	}
	return @uris;
	#original line that foreach loop replaced:
