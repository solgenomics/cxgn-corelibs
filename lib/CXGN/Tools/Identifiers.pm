...
=item sgn_locus_sequence

An SGN locus sequence identifier like 'Potato_SGNlocusID_2206_AM040153'
which is parsed as { id => 2206, species => 'Potato', ext_id => 'AM040153' }
...
A genbank identifier denoting a sequence accession, containing a
namespace and an identifier, such as gb|EAT47485.1|.

=item genbank_cdd

A genbank identifier denoting an entry in the NCBI Conserved Domain
Database (CDD).

=item swissprot_accession

A uniprot identifier composed by 'sp'+pipe+uniprot_accession+pipe+entry_name
for example sp|P22142|NDUS2_NEUCR means accession=P22142 and entry_name=NDUS2_NEUCR
(sp means swissprot)

=item uniref_accession

A uniprot accession is contains a combination of number and letters, 
for example: UniRef90_P22142, the accsssion will be P22142
...
sub link_identifier {
  ...
  my $url = identifier_url($ident,$ns)
    or return;
  #clean up the identifier if we can
  my $fns = ucfirst join ' ',split /_/,$ns;
  return qq|<a title="search for this $fns" href="$url">$ident</a>|;
}
...
######### sgn_locus_sequence
sub is_sgn_locus_sequence {
  return 1 if shift =~ /[a-z]+_SGNlocusID_\d+_.+/i;
  return 0;
}
sub url_sgn_locus_sequence {
  my ($id) = shift =~ /SGNlocusID_(\d+)/i;
  return "/phenome/locus_display.pl?locus_id=$id";
}
sub clean_sgn_locus_sequence {
  my $p = parse_sgn_locus_sequence(shift);
  return join'_',$p->{species},'SGNlocusID',$p->{id},$p->{ext_id};
}
sub parse_sgn_locus_sequence {
  my ($species,undef,$id,$extid) = split /_/,shift,4;
  return { species => ucfirst($species),
	   id  => $id+0,
	   ext_id => $extid,
	 };
}
...
sub is_swissprot_accession {
    my ($ident) = @_;
    return 1 if $ident =~ /^sp\|\w+\|\w+_\w+$/i;
}
sub url_swissprot_accession {
    my ($ident) = @_;
    $ident = clean_swissprot_accession($ident) or return;
    return "http://www.uniprot.org/uniprot/" . $ident;
}
sub clean_swissprot_accession {
    my ($ident) = @_;
    my $clean;
    if ($ident =~ m/^sp\|(\w+)\|\w+_\w+$/i) {
	$clean = $1;
    } else {
	$clean=$ident;
    }
...
sub parse_swissprot_accession {
    my ($ident)=@_;
    unless ($ident =~ m/^sp\|(\w+)\|\w+_\w+$/i) {
	return { id => $1};
    } else {
	return;
    }
...
#uniprot_uniref_accession
sub is_uniref_accession {
    my ($ident) = @_;
    return 1 if $ident =~ m/^UniRef\d+_\w+$/i;
}
sub url_uniref_accession {
    my ($ident) = @_;
    $ident = clean_uniref_accession($ident) or return;
    return "http://www.uniprot.org/uniprot/".$ident;
}
sub clean_uniref_accession {
    my ($ident) = @_;
    my $clean;
    if ($ident =~ m/^UniRef\d+_(\w+)$/i) {
	$clean = $1;
    } else {
	$clean=$ident;
    }
...
sub parse_uniref_accession {
    my ($ident)=@_;
    unless ($ident =~ m/^UniRef\d+_(\w+)$/i) {
	return { id => $1};
    } else {
	return;
    }
...
sub is_genbank_cdd {
  my ($ident) = @_;
  return 1 if $ident =~ /^(gnl\|)?cdd[\|:]\d+[\|:]?$/i;
  return 0;
}
sub url_genbank_cdd {
  my ($ident) = @_;
  $ident = clean_genbank_cdd($ident) or return;
  my $p = parse_genbank_cdd($ident) or return;
 return "http://www.ncbi.nlm.nih.gov/sites/entrez/query.fcgi?db=cdd&term=$p->{id}";
}
sub clean_genbank_cdd {
  my ($ident) = @_;
  if($ident =~ /(\d+)/i) {
    return "gnl|CDD|$1";
  } else {
...
sub url_go_term {
  ...
  return "http://www.geneontology.org/cgi-bin/chooser.cgi?search_query=$urlencode{$ident}&search_constraint=terms";
}
sub clean_go_term {
  my ($ident) = @_;
  my @d = $ident =~ /\d+/g;
