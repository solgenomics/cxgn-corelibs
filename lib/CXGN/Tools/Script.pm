...
=head2 lock_script
  ...
  Args : quiet => 0, #if passed, don't warn if script is already running
...
sub lock_script {
  ...
  open( my $lock_fh, ">$lockfile_name" )
    or croak "Could not open '$lockfile_name' for writing";
  print $lock_fh "$PROCESS_ID\n";
  close $lock_fh;

  -f $lockfile_name or croak "Could not lock script with pidfile '$lockfile_name'\n";
  return 1;
}
sub _read_lockfile {
  open( my $lock_fh, $lockfile_name )
...
=head2 unlock_script
  ...
  Args : none
  Ret : 0 on failure, 1 on successful release, 2 if the lock file did
        not belong to this script and thus was not deleted
  Side Effects:
  Example:
...
sub unlock_script {
  #delete our lockfile
  my $pid = _read_lockfile();
  return 2 unless $pid && $pid == $PROCESS_ID;
  unlink( $lockfile_name );
  if( -f $lockfile_name ) {
    warn "Could not delete lockfile '$lockfile_name': $!";
    return 0;
  }
  return 1;
}
# try to clear any locks when the program ends
END {
  unlock_script();
  1;
}
