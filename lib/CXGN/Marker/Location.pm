...
sub lg_name
{
    ...
    if($lg_name)
    {
        ...
        unless($lg_id)
        {
            croak"Linkage group '$lg_name' does not exist on map_version_id '$self->{map_version_id}'";
        } 
        $self->{lg_id}=$lg_id;       
...
sub lg_id { 
    ...
    if ($lg_id) { 
	unless ($self->{map_version_id}) { 
	    croak "You must set map_version_id before trying to set lg_id. Thanks!\n";
	}
	$self->{lg_id}=$lg_id;
    }
    return $self->{lg_id};
...
sub confidence
{
    ...
    {
        my $confidence_id;
        $confidence_id=CXGN::Marker::Tools::get_marker_confidence_id($self->{dbh},$confidence);
...
sub exists
{
    ...
    {
        croak"Cannot test for a location's existence without knowing which marker it goes with--store marker and set experiment's marker ID before storing locations";
    }
    unless($self->{lg_id})
    {
        croak"You really should have an lg_id set before testing for a location's existence";
    }
    unless($self->{map_version_id})
    {
        croak"You really should have a map_version_id set before testing for a location's existence";        
    }
    unless(defined($self->{position}))
    {
        croak"You really should have a position set before testing for a location's existence";        
    }
    unless(defined($self->{confidence_id}))
    {
        croak"You really should have a confidence_id set before testing for a location's existence";        
    }
    if($self->{location_id})
    {
    ...
    ');
    $q->execute($self->{marker_id},$self->{lg_id},$self->{map_version_id},$self->{position},$self->{confidence_id},$self->{subscript});
    my %found_location_ids;#a place to keep all location IDs that match for this marker, for use in error checking in a moment
    my($location_id)=$q->fetchrow_array();
    if($location_id)#if we found some matching locations for this marker
    {
        $self->{location_id}=$location_id;#get the ID of the existing row in the database so we know we've already been stored
        $found_location_ids{$location_id}=1;#make a note of the location ID found
        while(my($other_location_id)=$q->fetchrow_array())#grab all other location IDs
        {
            $found_location_ids{$other_location_id}=1;
        }
        if(keys(%found_location_ids)>1)#if we found more than one matching location ID, then the database data is not how we expect it to be
        {
...
sub exists_with_any_confidence
{
    ...
    {
        croak"You have a confidence_id set--why not just use the 'exists' function instead?";        
    }
    if($self->{location_id})
    {
        #warn"I think it's pretty obvious that this location exists, since it seems to have been loaded from the database, or recently stored to the database--it already has an id of $self->{location_id}";
    ...
    ('    
        ...
        where
            ...
            and not(subscript is distinct from ?)  
    ');
    $q->execute($self->{marker_id},$self->{lg_id},$self->{map_version_id},$self->{position},$self->{subscript});
    my %found_location_ids;#a place to keep all location IDs that match for this marker, for use in error checking in a moment
    my($location_id)=$q->fetchrow_array();
    if($location_id)#if we found some matching locations for this marker
    {
...
sub as_string
{
    my $self=shift;
    my $string="<location>\n";
    $string.="\tmarker_id: '$self->{marker_id}'\tsubscript: '$self->{subscript}'\n";
    $string.="\tlg_name: '$self->{lg_name}'\tlg_id: '$self->{lg_id}'\tposition: '$self->{position}'\n";
    $string.="\tlocation_id: '$self->{location_id}'\tmap_version_id: '$self->{map_version_id}'\n";
    $string.="\tconfidence: '$self->{confidence}'\tconfidence_id: '$self->{confidence_id}'\n";
    $string.="</location>\n";
    return $string;
}
