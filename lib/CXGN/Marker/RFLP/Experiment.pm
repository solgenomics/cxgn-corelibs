...
sub vector
{
    ...
    if($value)
    {    
        unless($value eq 'pBLUESC' or $value eq 'pBR' or $value eq 'pCDNAII' or $value eq 'pCR1000' or $value eq 'pCRII' or $value eq 'PCRII' or $value eq 'pGEM4Z' or $value eq 'pUC')
        {
...
sub cutting_site
{
    ...
    if($value)
    {    
        unless($value eq 'EcoR1' or $value eq 'EcoR1/BamH1' or $value eq 'EcoR1/HindIII' or $value eq 'EcoRI' or $value eq 'ECORI' or $value eq 'HindIII/EcoR1' or $value eq 'PST1' or $value eq 'SRD/PST1')
        {    
...
sub forward_seq_id
{
    ...
    if($value)
    {    
        my $q=$self->{dbh}->prepare('select seq_id from rflp_sequences where seq_id=?');
        $q->execute($value);
...
sub drug_resistance
{
    ...
    if($value)
    {    
        unless($value eq 'Amp' or $value eq 'Amp*' or $value eq 'AMP' or $value eq 'KN' or $value eq 'TET')
        {   
...
sub equals
{
    ...
    if
    (
        $self->{marker_id} eq $other->{marker_id}
...
sub store_unless_exists
{
    ...
    print"INSERTING:\n".$self->as_string();
    $self->{rflp_id}=$self->{dbh}->last_insert_id('rflp_markers') or die("Could not get last insert ID from database");
    return $self->{rflp_id};
}
sub as_string
{
