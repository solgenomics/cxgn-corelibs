...
"SELECT stock.stock_id,band_size,multiple_flag FROM sgn.pcr_exp_accession inner join sgn.pcr_product using(pcr_exp_accession_id) inner join public.stock on(pcr_exp_accession.stock_id=stock.stock_id) WHERE enzyme_id is null and pcr_experiment_id=?"
        ...
        if ( $sizes->[0] ) {
            $self->{pcr_bands} = $self->query_results_to_bands_hash($sizes);
        }
        $q = $dbh->prepare(
"SELECT stock.stock_id,band_size,multiple_flag FROM sgn.pcr_exp_accession inner join sgn.pcr_product using(pcr_exp_accession_id) inner join public.stock on(pcr_exp_accession.stock_id=stock.stock_id) WHERE enzyme_id is not null and pcr_experiment_id=?"
        );
        $q->execute( $self->{pcr_experiment_id} );
        $sizes = $q->fetchall_arrayref();
        if ( $sizes->[0] ) {
        ...
        #get enzyme
        $q = $dbh->prepare(
"SELECT enzyme_id,enzyme_name FROM pcr_exp_accession inner join pcr_product using(pcr_exp_accession_id) inner join enzymes using(enzyme_id) where pcr_experiment_id=?"
        );
        $q->execute( $self->{pcr_experiment_id} );
        ( $self->{enzyme_id}, $self->{enzyme} ) = $q->fetchrow_array()
          ; #only fetching one row, because they all should be the same. there should be both db and api constraints ensuring that.
    }
...
sub check_pcr_band_arrays {
    my ( $stock, $pcr_hash_1, $pcr_hash_2 ) = @_;

    my $comp = Array::Compare->new();

    my $croaking =
"PCR bands (or digest bands) entry for stock '$stock' does not appear to be array ref";

    unless ( $pcr_hash_1->{$stock} and $pcr_hash_2->{$stock} ) { return 0 }
    unless ( ( ref( $pcr_hash_1->{$stock} ) eq 'ARRAY' ) ) { croak $croaking }
    unless ( ( ref( $pcr_hash_2->{$stock} ) eq 'ARRAY' ) ) { croak $croaking }
...
# Array::Compare::perm returns true if lists are the same or permutations of each other (bands may have been stored in any order)
    unless ( $comp->perm( $pcr_hash_1->{$stock}, $pcr_hash_2->{$stock} ) ) {
        return 0;
    }
...
sub equals {
    ...
    unless ( $other->isa('CXGN::Marker::PCR::Experiment') ) {
        croak "Must send in a PCR experiment object to equals function";
    }
...
"Must set both PCR experiment objects' marker IDs before comparing\n-----\nself:\n"
    ...
    unless ( $self->protocol() and $other->protocol() ) {
        croak
...
"Can't check for equality; invalid predicted field for other object:\n"
    ...
    # remove empty keys
    for my $k ( keys( %{$pcr_hash_1} ) ) {
        delete $pcr_hash_1->{$k} if ( @{ $pcr_hash_1->{$k} } == 0 );
    }
    for my $k ( keys( %{$pcr_hash_2} ) ) {
        delete $pcr_hash_2->{$k} if ( @{ $pcr_hash_2->{$k} } == 0 );
    }

    # check pcr band arrays for all stocks in first object
    for my $stock ( keys( %{$pcr_hash_1} ) ) {
        unless ( &check_pcr_band_arrays( $stock, $pcr_hash_1, $pcr_hash_2 ) ) {
            return 0;
        }
    }

# then check pcr band arrays for all stocks in second object, in case the second has stocks the first doesn't
    for my $stock ( keys( %{$pcr_hash_2} ) ) {
        unless ( &check_pcr_band_arrays( $stock, $pcr_hash_1, $pcr_hash_2 ) ) {
            return 0;
        }
    }
    ...
    # check pcr digest band arrays for all stocks in first object
    for my $stock ( keys( %{$pcr_hash_1} ) ) {
        unless ( &check_pcr_band_arrays( $stock, $pcr_hash_1, $pcr_hash_2 ) ) {
            return 0;
        }
...
# then check pcr digest  band arrays for all accessions in second object, in case the second has accessions the first doesn't
 ...
 #only compare mg and temp IF they are present in BOTH objects... see note below
    if (    ( $self->mg_conc() and $other->mg_conc() )
        and ( $self->mg_conc() != $other->mg_conc() ) )
    {
        return 0;
    }
    if (    ( $self->temp() and $other->temp() )
...
#warn"I think it's pretty obvious that this experiment exists, since it seems to have been loaded from the database, or recently stored to the database--it already has an id of $self->{pcr_experiment_id}";
    ...
    unless ( $self->predicted() eq 'f' or $self->predicted() eq 't' ) {
        croak "Can't check for existence; invalid predicted field for self:\n"
          . $self->as_string();
    }
    ...
    while ( my ($possible_match_id) =
        $possible_matches_query->fetchrow_array() )
    {

        #print"possible match id: $possible_match_id\n";
        my $experiment_for_comparison =
          CXGN::Marker::PCR::Experiment->new( $self->{dbh},
            $possible_match_id );
        if ( $self->equals($experiment_for_comparison) ) {
            $self->{pcr_experiment_id} = $experiment_for_comparison
              ->{pcr_experiment_id}; #ok, we've been found to already exist, so set our pcr_experiment_id
            return $self->{pcr_experiment_id};
        }
    }
    return;
...
sub store_unless_exists {
    ...
    if ( $self->exists() ) { 
	print STDERR "This pcr experiment has already been stored.\n";
	return;
    }

    unless ( $self->{marker_id} ) {
        croak "Cannot store experiment without marker ID";
    }
    unless ( $self->{protocol} ) {
        croak
"Cannot store experiment without protocol. Use 'unknown' if necessary.";
    ...
    unless ( $self->predicted() eq 'f' or $self->predicted() eq 't' ) {
        croak "Can't store; invalid predicted field for self:\n"
          . $self->as_string();
    }
    if ( $self->{pcr_experiment_id} ) {
        croak
"This experiment appears to have been stored already or created from an existing database entry";
 ...
 #like lukas's modifiable form object.
    if ( $self->{pcr_digest_bands} ) {
        unless ( $self->{enzyme_id} ) {
            croak "Must have an enzyme set to store digest bands";
        }
    }
    ...
    if ( $self->fwd_primer() ) {
        my $fwd_info =
          $sql->insert_unless_exists( 'sequence',
            { 'sequence' => $self->fwd_primer() } );
        $self->{fwd_primer_id} = $fwd_info->{id};
    }
    if ( $self->rev_primer() ) {
        my $rev_info =
          $sql->insert_unless_exists( 'sequence',
            { 'sequence' => $self->rev_primer() } );
    ...
    $self->{pcr_experiment_id} = $self->{dbh}->last_insert_id('pcr_experiment')
      or croak "Could not get last_insert_id from pcr_experiment";
    ...
    my $exp_acc_insert =
      $self->{dbh}->prepare(
'insert into sgn.pcr_exp_accession (pcr_experiment_id,stock_id) values (?,?)'
      );
    my $pcr_band_insert =
      $self->{dbh}->prepare(
'insert into sgn.pcr_product (pcr_exp_accession_id,enzyme_id,multiple_flag,band_size,predicted) values (?,?,?,?,?)'
      );



    for my $stock_id ( keys(%stocks) ) {
        $exp_acc_insert->execute( $self->{pcr_experiment_id}, $stock_id );
        $stocks{$stock_id} = $self->{dbh}->last_insert_id('pcr_exp_accession')
          or croak "Could not get last_insert_id from pcr_exp_accession";

        my @stock_pcr_bands;
        my @stock_pcr_digest_bands;
        if ( $self->{pcr_bands}->{$stock_id} ) {
            @stock_pcr_bands = @{ $self->{pcr_bands}->{$stock_id} };
        }
        if ( $self->{pcr_digest_bands}->{$stock_id} ) {
            @stock_pcr_digest_bands =
              @{ $self->{pcr_digest_bands}->{$stock_id} };
        }
        if ( $stock_pcr_bands[0] ) {   #if there is at least one value in the pcr bands list for this stock
            for my $band (@stock_pcr_bands) {
                ...
                if ( $band =~ /^m/i ) {
                    $pcr_band_insert->execute( $stocks{$stock_id}, undef, 1,
                        undef, $self->{predicted} );
                }
                else {

                    $pcr_band_insert->execute( $stocks{$stock_id}, undef, undef,
                        $band, $self->{predicted} );
                }
            }
        }
        if ( $stock_pcr_digest_bands[0] ) { 
            # if there is at least one value in the pcr digest bands list for this stock
            #if the band entry starts with an m, it means multiple bands, so set the multiple flag.
            for my $band (@stock_pcr_digest_bands) {
                if ( $band =~ /^m/i ) {
                    $pcr_band_insert->execute( $stocks{$stock_id},
                        $self->{enzyme_id}, 1, undef, $self->{predicted} );
                }
                else {
                    $pcr_band_insert->execute( $stocks{$stock_id},
                        $self->{enzyme_id}, undef, $band, $self->{predicted} );
                }
            }
        }
    }
    ...
    if ( my $oops_id = $self->store_unless_exists() ) {
        my $croaking =
"Oops, this object isn't working correctly. Immediately after being stored with ID "
...
"This experiment object does not appear to have been loaded or inserted into the database yet, so you cannot update its enzymes";
    ...
    my $sth =
      $self->{dbh}->prepare(
'update pcr_experiment set additional_enzymes=? where pcr_experiment_id=?'
      );
...
sub as_string {
    ...
    if ( $self->{marker_id} ) {
        @marker_names =
          CXGN::Marker->new( $self->{dbh}, $self->{marker_id} )
          ->name_that_marker();
    }
    else {
    ...
    if ( $self->{location_id} ) {
        $string .=
          "\tThis is a mapping experiment; location ID: $self->{location_id}\n";
    }
    else {
        $string .=
"\tThis experiment does not yet have a map location associated with it in the marker_experiment table\n";
    }
    $string .= "\tProtocol: $self->{protocol}\n";
    $string .=
      "\tPrimers: $self->{fwd_primer} (fwd)\t\t$self->{rev_primer} (rev)\n";
...
sub query_results_to_bands_hash {
    ...
    my %bands;
    for my $row ( @{$sizes} ) {
        my ( $stock, $band_size, $multiple_flag ) = @{$row};
        if ( $stock and ( $band_size or $multiple_flag ) ) {
            my $insert_value;
            if ($band_size) {
...
sub join_bands_hash {
    ...
    for my $stock ( keys( %{$bands_hash} ) ) {
        $expected_structure{$stock} = join( '+', @{ $bands_hash->{$stock} } );
    }
...
sub temp {
    ...
    if ($value) {
        ...
        if ( $value =~
            s/F$//i )    #if it was an F for Fahrenheit, convert it to Celsius
        {
            $value = ( $value + 40 ) * 5 / 9;
        }
...
sub enzyme {
    ...
    if ($enzyme) {
        ...
        unless ( $self->{enzyme_id} =
            CXGN::Marker::Tools::get_enzyme_id( $self->{dbh}, $enzyme ) )
        {
            croak
...
=head2 add_pcr_bands_for_stock

    $experiment->add_pcr_bands_for_stock('250+400','LA716');

    In a change from the previous version, this actually hard stores
    the band info in the database. It only inserts a band size once.

    Note: add_pcr_bands_for_accession is deprecated.
...
sub add_pcr_bands_for_accession {
    warn "add_pcr_bands_for_accession is deprecated\n";
    shift->add_pcr_bands_for_stock(@_);
}

sub add_pcr_bands_for_stock {
    my $self = shift;
    ...
    #    {
    #        croak"Accession '$stock_name' not found\n";
    #    }

    if (!$self->{pcr_experiment_id}) { 
	die "Need a pcr_experiment_id!!!\n";
    }

    my $stock_id = $self->get_stock_id_with_stock_name($stock_name);

    #my $accession_id=$accession_object->accession_id();
    unless ($stock_id) { croak("Accession '$stock_name' not found"); }
    my @bands = split( /\+/, $bands_string );
    $self->{pcr_bands}->{$stock_id} = \@bands;
    $self->{pcr_bands} = $self->test_and_clean_bands( $self->{pcr_bands} );
    
#    print STDERR "ADDING BANDS ".(@bands)."\n";
    foreach my $band (@bands) { 
	my $sth = $self->{dbh}->prepare("SELECT marker_id, pcr_exp_accession.pcr_exp_accession_id, pcr_product_id FROM pcr_experiment JOIN pcr_exp_accession USING(pcr_experiment_id) LEFT JOIN pcr_product USING (pcr_exp_accession_id) WHERE band_size=? AND pcr_exp_accession.stock_id=? AND marker_id=?");
	$sth->execute($band, $stock_id, $self->marker_id());
	my ($marker_id, $pcr_exp_accession_id, $pcr_product_id) = $sth->fetchrow_array();

	if ( $pcr_exp_accession_id) { 
	    print STDERR "pcr_exp_accession for $stock_id is already in the database -- not storing\n"; 
	}
	else { 
	    my $p1 = $self->{dbh}->prepare("INSERT INTO sgn.pcr_exp_accession (pcr_experiment_id, stock_id) VALUES (?, ?) RETURNING pcr_exp_accession_id");
	    $p1->execute($self->{pcr_experiment_id}, $stock_id);
	    ($pcr_exp_accession_id) = $p1->fetchrow_array();
	    
	}
	

	if ($pcr_product_id ) { 
	    print STDERR "pcr_product of size $band for stock $stock_id already in database -- not storing.\n";
	}
	else { 
	    my $new_pcr_product_id = $self->{dbh}->do("INSERT INTO sgn.pcr_product (pcr_exp_accession_id, band_size) VALUES (?, ?) RETURNING pcr_product_id", {}, $pcr_exp_accession_id, $band);
...
=head2 add_pcr_digest_bands_for_stock

    $experiment->add_pcr_digest_bands_for_stock('250+400','LA716');

   (needs fixing, see add_digest_bands_for_stock)

    Side effect: Dies if the stock is not valid.
    Note:        add_pcr_digest_bands_for_accession is deprecated.
...
sub add_pcr_digest_bands_for_accession {
    ...
    unless ($stock_id) { croak("Accession '$stock_name' not found"); }
    my @bands = split( /\+/, $bands_string );
    $self->{pcr_digest_bands}->{$stock_id} = \@bands;
    $self->{pcr_digest_bands} =
      $self->test_and_clean_bands( $self->{pcr_digest_bands} );
}

sub get_stock_id_with_stock_name {
    my $self       = shift;
    my $stock_name = shift;

    my $sth =
      $self->{dbh}->prepare("SELECT stock_id FROM stock where name=?");
    $sth->execute($stock_name);
...
sub test_and_clean_primer {
    ...
    unless (
        $primer =~ /[ATGCatgc]+/ ) #primers are known base pairs start to finish
    {
...
sub test_and_clean_bands {
    ...
    for my $stock_id ( keys( %{$bands} ) ) {

#       unless(CXGN::Accession->new($self->{dbh},$accession_id)->accession_id()){croak"Accession '$accession_id' not found";}

        unless ( $self->_valid_stock_id($stock_id) ) {
            croak "stock_id $stock_id is not valid!";
        }

        unless ( ref( $bands->{$stock_id} ) eq 'ARRAY' ) {
            croak "Bands hash ref must contain array refs";
        }
        my @bands_array =
          @{ $bands->{$stock_id}
          }; #copy this array out to make the following code more readable, maybe
        for my $index ( 0 .. $#bands_array ) {
            $bands_array[$index] = CXGN::Tools::Text::remove_all_whitespaces(
                lc( $bands_array[$index] ) );

            if ( $bands_array[$index] =~ /^m/i ) {
                $bands_array[$index] = 'Multiple';
            }
            ...
            unless ( ( $bands_array[$index] eq 'Multiple' )
                or ( CXGN::Tools::Text::is_number( $bands_array[$index] ) ) )
            {
                croak "'$bands_array[$index]' is an invalid band size";
            }
        }
        $bands->{$stock_id} = \@bands_array;    #copy this array back in
    }
    return $bands;
}
...
sub store_sequence {
    ...
    my $seq           = shift
      || die
'No sequence for type $sequence_type passed to store_sequence function! \n';
...
"Sequence type $sequence_type does not exist in the database!\n Expected to find cvterm $sequence_type with cv_name 'sequence'!\n Please check your databae, and make sure Sequence Ontology is up-to-date\n "
    ...
    my $sql = CXGN::DB::SQLWrappers->new( $self->{dbh} );
    my $sequence =
      $sql->insert_unless_exists( 'sequence', { 'sequence' => $seq } );

    #does the link exist?
    my @ids = $sql->select(
        'pcr_experiment_sequence',
        {
            sequence_id       => $sequence->{id},
            pcr_experiment_id => $self->{pcr_experiment_id},
            type_id           => $type_id
        }
    );
    if ( !@ids ) {
        ...
        #store the link
        $q =
"Insert INTO sgn.pcr_experiment_sequence (sequence_id, pcr_experiment_id, type_id)
