package CXGN::Garbage::physical_db_tools;
...
my %hosts = ('henbane' => {'user' => 'robert', 'password' => 'jasminesaville', 'db' => 'Physical2'},
	     'amatxu' => {'user' => 'koni', 'password' => 'bitchbadass', 'db' => 'physical'},
	     'zamolxis' => {'user' => 'web_usr', 'password' => 'tomato', 'db' => 'physical'});
...
sub connect_physical_db () {
    ...
    # Work out connection settings based on the machine we're working on.
    my $konnex = &auto_configure_db_settings();
#    my %settings = %$konnex;
    my %settings;
    my $dsn = "dbi:mysql:physical";
#    if ($settings{'host'}) { $dsn .= $settings{'host'} . ".sgn.cornell.edu"; }
    my $dbh = DBI->connect($dsn, 'web_usr', 'tomato', { RaiseError => 1});
    # Increase max buffer size.
    my $newsize = 20480;
    ($dbh->{LongReadLen} = $newsize) 
	|| die "Error setting LongReadLen to $newsize: $DBI::errstr";
    # Troubleshooting only.
    #print STDERR "Connected to " . ($settings{'host'} || "localhost") . " as $settings{'user'} with passwd $settings{'password'}.\n";
    return $dbh;
...
sub get_current_overgo_version ($) {
    ...
    my ($dbh) = @_;
    my $stm = "SELECT overgo_version FROM overgo_version WHERE current=1";
...
sub get_current_overgo_version_and_updated_on ($) {
    ...
    my ($dbh) = @_;
    my $sth = $dbh->prepare("SELECT overgo_version, updated_on FROM overgo_version WHERE current=1");
    $sth->execute;
    my ($overgo_v, $updated) = $sth->fetchrow_array;
    $sth->finish;
...
sub get_latest_overgo_version ($;$) {
    ...
    my ($dbh, $userid) = @_;
    my $stm = $userid ? "SELECT overgo_version FROM overgo_version WHERE updated_by=$userid ORDER BY overgo_version DESC" : "SELECT overgo_version FROM overgo_version ORDER BY overgo_version DESC"; 
    my $sth = $dbh->prepare($stm);
    $sth->execute;
    my $version = $sth->fetchrow_array;
    $sth->finish;
...
sub get_current_fpc_version ($) {
    ...
    my ($dbh) = @_;
    my $sth = $dbh->prepare("SELECT fpc_version FROM fpc_version WHERE current=1");
    $sth->execute;
...
sub get_current_fpc_version_and_date ($) {
    ...
    my ($dbh) = @_;
    my $sth = $dbh->prepare("SELECT fpc_version, updated_on FROM fpc_version WHERE current=1");
    $sth->execute;
    my ($fpc_version, $date) = $sth->fetchrow_array;
    $sth->finish;
...
sub get_plate_number_by_plate_id ($$) {
    ...
    my ($dbh, $plate_id) = @_;
    my $sth = $dbh->prepare("SELECT plate_number FROM overgo_plates WHERE plate_id=?");
...
sub get_total_number_of_bacs ($) {
    ...
    my ($dbh) = @_;
    my $sth = $dbh->prepare("SELECT COUNT(bac_id) FROM bacs WHERE bad_clone!=1");
    $sth->execute;
...
sub count_all_bacs_which_hit_all_plates ($;$) {
    ...
    my $sth = $dbh->prepare("SELECT COUNT(DISTINCT b.bac_id) FROM bacs AS b INNER JOIN overgo_associations AS oa ON b.bac_id=oa.bac_id INNER JOIN probe_markers AS pm ON oa.overgo_probe_id=pm.overgo_probe_id WHERE oa.overgo_version=?");
...
sub count_all_bacs_which_hit_plate_n ($$;$) {
    ...
    $overgo_version ||= &get_current_overgo_version($dbh);
    my $sth = $dbh->prepare("SELECT COUNT(DISTINCT b.bac_id) FROM bacs AS b INNER JOIN overgo_associations AS oa ON b.bac_id=oa.bac_id INNER JOIN probe_markers AS pm ON oa.overgo_probe_id=pm.overgo_probe_id INNER JOIN overgo_plates AS op ON pm.overgo_plate_id=op.plate_id WHERE oa.overgo_version=? AND op.plate_number=?");
...
sub count_all_bacs_which_plausibly_hit_all_plates ($;$) {
    ...
    my ($dbh, $overgo_version) = @_;
    $overgo_version ||= &get_current_overgo_version($dbh);
    my $sth = $dbh->prepare("SELECT COUNT(DISTINCT b.bac_id) FROM bacs AS b INNER JOIN overgo_associations AS oa ON b.bac_id=oa.bac_id INNER JOIN probe_markers AS pm ON oa.overgo_probe_id=pm.overgo_probe_id WHERE oa.overgo_version=? AND oa.plausible=1");
    $sth->execute($overgo_version);
    my $bac_count = $sth->fetchrow_array;
    $sth->finish;
    return ($bac_count || 0);
...
sub count_all_bacs_which_plausibly_hit_plate_n ($$;$) {
    ...
    my ($dbh, $plateno, $overgo_version) = @_;
    $overgo_version ||= &get_current_overgo_version($dbh);
    my $sth = $dbh->prepare("SELECT COUNT(DISTINCT b.bac_id) FROM bacs AS b INNER JOIN overgo_associations AS oa ON b.bac_id=oa.bac_id INNER JOIN probe_markers AS pm ON oa.overgo_probe_id=pm.overgo_probe_id INNER JOIN overgo_plates AS op ON pm.overgo_plate_id=op.plate_id WHERE oa.overgo_version=? AND oa.plausible=1 AND op.plate_number=?");
    $sth->execute($overgo_version, $plateno);
    my $bac_count = $sth->fetchrow_array;
    $sth->finish;
    return ($bac_count || 0);
...
sub count_wells_with_plausible_hits_on_plate_n ($$;$) {
    ...
    my ($dbh, $plateno, $overgo_version) = @_;
    $overgo_version ||= &get_current_overgo_version($dbh);
    my $sth = $dbh->prepare("SELECT COUNT(DISTINCT oa.overgo_probe_id) FROM overgo_plates AS op INNER JOIN probe_markers AS pm ON op.plate_id=pm.overgo_plate_id INNER JOIN overgo_associations AS oa ON pm.overgo_probe_id=oa.overgo_probe_id WHERE op.plate_number=? AND oa.overgo_version=? AND oa.plausible=1");
    $sth->execute($plateno, $overgo_version);
...
sub count_distinct_anchor_points_on_map_chromosome ($$$;$) {
    ...
    $overgo_version ||= &get_current_overgo_version($dbh);
    my $sth = $dbh->prepare("SELECT COUNT(DISTINCT md.loc_id) FROM overgo_associations AS oa INNER JOIN probe_markers AS pm ON oa.overgo_probe_id=pm.overgo_probe_id INNER JOIN sgn.marker_locations AS ml ON pm.marker_id=ml.marker_id INNER JOIN sgn.mapdata AS md ON ml.loc_id=md.loc_id INNER JOIN linkage_groups as lg USING(lg_id) WHERE oa.overgo_version=? AND md.map_id=? AND lg_name=?");
    $sth->execute($overgo_version, $map_id, $chromonum);
...
sub get_plate_as_hash ($$;$) {
    ...
    my $plate_stats_sth = $dbh->prepare("SELECT plate_id, row_max, col_max FROM overgo_plates WHERE plate_number=?");
    ...
    my $platemarkers_stm = $overgo_version ? "SELECT m.marker_name, pm.overgo_plate_row, pm.overgo_plate_col FROM probe_markers AS pm LEFT JOIN sgn.markers AS m ON pm.marker_id=m.marker_id LEFT JOIN deprecated_probes AS dp ON (pm.overgo_probe_id=dp.overgo_probe_id AND dp.overgo_version=$overgo_version) WHERE pm.overgo_plate_id=? AND dp.dp_id IS NULL ORDER BY pm.overgo_plate_row, pm.overgo_plate_col" : "SELECT m.marker_name, pm.overgo_plate_row, pm.overgo_plate_col FROM probe_markers AS pm LEFT JOIN sgn.markers AS m ON pm.marker_id=m.marker_id WHERE pm.overgo_plate_id=? ORDER BY pm.overgo_plate_row, pm.overgo_plate_col";
...
sub new_overgo_version ($$) {
    ...
    my $version_sth = $dbh->prepare("INSERT INTO overgo_version SET updated_by=?, updated_on=?");
...
sub new_fpc_version ($$$$;$) {
    ...
    my $version_sth = $dbh->prepare("INSERT INTO fpc_version SET updated_on=?, updated_by=?, current=0, fpcfile=?, comments=?");
...
sub set_current_fpc_version ($$) {
    ...
    my ($dbh, $new_cv) = @_;
    $new_cv =~ /^\d+$/ || return 0;
    $dbh->do("UPDATE fpc_version SET current=0");
    $dbh->do("UPDATE fpc_version SET current=1 WHERE fpc_version=$new_cv");
    return $new_cv;
...
sub remove_plate_by_plate_id ($$) {
    ...
    my @probemarkers;
    my $probe_sth = $dbh->prepare("SELECT overgo_probe_id overgo_probe_id FROM probe_markers WHERE overgo_plate_id=?");
    ...
    my $clear_overgo_assocs_sth = $dbh->prepare("DELETE FROM overgo_associations WHERE overgo_probe_id=?");
    my $clear_tentative_overgo_assocs_sth = $dbh->prepare("DELETE FROM tentative_overgo_associations WHERE overgo_probe_id=?");
...
sub clear_overgo_associations ($$) {
    ...
    my @toa=();
    my $get_toa_ids_sth = $dbh->prepare("SELECT tentative_assoc_id FROM tentative_overgo_associations WHERE overgo_version=?");
    ...
    my $clear_tacg_sth = $dbh->prepare("DELETE FROM tentative_association_conflict_groups WHERE tentative_assoc_id=?");
    foreach (@toa) {
	$clear_tacg_sth->execute($_);
    }
    $clear_tacg_sth->finish;
    # Now bulk DELETE from overgo_associations and tentative_overgo_associations.
    $dbh->do("DELETE FROM overgo_associations WHERE overgo_version=$overgo_version");
    $dbh->do("DELETE FROM tentative_overgo_associations WHERE overgo_version=$overgo_version");
