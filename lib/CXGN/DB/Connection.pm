...
=head2 new
  ...
  Args: parameters hash ref as
        ({
            REQUIRED:
            config => hashref of configuration parameters (from a config object or similar)

            OPTIONAL (override parameters in config of the same name):
...
sub new_no_connect {
  ...
  #carp "WARNING: CXGN::DB::Connection is deprecated"
  #    unless $ENV{TESTING_CXGN_DB_CONNECTION};
  ...
  if (ref ($db)) { # The first argument was a hash ref.
    if (ref $p) { # Something's wrong if TWO hash refs were passed
      croak ("invalid arguments to CXGN::DB::Connection constructor");
    } else {
  ...
  # process the config object we were passed
  my $conf = delete $p->{config};
  if( $conf ) {
      # if we got a config object, check it for validity
      if( Scalar::Util::blessed($conf) && $conf->isa('Bio::GMOD::DB::Config') ) {
          $conf = { dbhost => $conf->host,
                    dbport => $conf->port,
                    dbuser => $conf->user,
  ...
  my $dbbranch_default = exists $conf->{"production_server"} && $conf->{"production_server"} ? "production" : "devel";

  if( $p->{dbschema} ) {
      carp "WARNING: dbschema argument to CXGN::DB::Connection is ignored.  Set the dbsearchpath configuration variable instead";
  }
...
#  warn "branch: " . $dbbranch_default;
  ...
  my @args = (# hash key,       environment var name,    configuration var name,      hardcoded default value
  ...
  foreach my $arg (@args) {
	...
	if defined $envname && ! defined $self->$argname;
      ...
      $self->$argname( $conf->{$confname} )
	if defined $confname && ! defined $self->$argname && exists $conf->{$confname};
...
sub new {
    ...
    # Now connect to a DB, dying from the caller's perspective
    eval { $self->_dbh( DBI->connect($self->get_connection_parameters) ) }; croak $@ if $@;
    $self->trace_msg('CXGN_TRACE | '._compact_backtrace().' | '.__PACKAGE__."::new | $self | ".$self->_dbh."\n",1);

    #generate the search path
    if ( $self->_dbtype eq 'Pg' ) {
        my $conf = $self->_conf;
        my $configured_search_path =
            Scalar::Util::blessed($conf)
                  ? $conf->get_conf('dbsearchpath')
                  : $conf->{dbsearchpath};

        $configured_search_path
            or croak 'no dbsearchpath conf variable set!  you must set one!';

        my @searchpath = ref $configured_search_path
            ? @$configured_search_path
            : split /\s*,\s*/, $configured_search_path;

        push @searchpath, 'tsearch2' unless $self->_dbh->{pg_server_version} >= 80300;
        $self->do("SET search_path TO ".join(',',@searchpath));
    }
...
sub qualify_schema {
  ...
  carp "qualify_schema method is deprecated";
...
sub base_schema {
    my ($self, $schema) = @_;
    carp "base_schema method is deprecated";
    return $schema;
}
...
sub last_insert_id {
  ...
  my ($seq) = $self->selectrow_array("SELECT split_part(replace(adsrc, 'nextval(''', ''), '''::', 1)
                                       ...
                                       WHERE t.relkind='r'                   -- select only tables
                                         ...
                                         AND n.nspname = '$schema'           -- select only schemas whose name is the schema argument");
  $seq or die"No sequence name found for table '$table' and schema '$schema' using qualified schema name '$schema'.\n";
  my ($id) = $self->selectrow_array("SELECT currval('$schema.$seq')")
      or die "No id found for table '$table' and schema '$schema' using qualified schema name '$schema' and sequence name '$seq'.\n";
  return ($id);
}
...
sub is_valid_dbh
{
    ...
    {
        warn "'$dbh' is a reference but does not look like a valid dbh";
        return 0;
    }
    return 1;
}
