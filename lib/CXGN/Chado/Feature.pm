...
sub store {
    ...
    my $feature_id= $self->get_feature_id();
    my $link=0; #boolean for inserting feature_dbxref  
	...
	$self->set_dbxref_id($dbxref_id);
	$link=1; #this means a feature_dbxref link has to be extablished, even if there is a feature_id
     }
    
    eval{
	#query for inserting a feature_dbxref to be executed if no feature_id or no dbxref_id
	...
	if (!$feature_id) {
	    ...
	    if ($link) { $feature_dbxref_sth->execute($feature_id, $dbxref_id);	 }
	}
    };
	
	if($@) {
	die "Feature.pm:store: An error occurred: $@";
    } 
    return $feature_id;
...
=head2 accessors get_message/set_message

 Usage: $self->set_message($message)
 Desc:  accessor used in Tools::FeatureFetch for returning an error message if NCBI server is down.
        Can be used in a similar way from any other class that returns a Feature object.
 Ret:   
 Args:  
 Side Effects: 
 Example:
...
sub feature_exists {
    ...
    my $query = "SELECT feature_id
                  ...
                  WHERE  feature.name ilike ? and dbxref_id is not null" ;
    my $sth = $self->get_dbh()->prepare($query);
...
=head2 associated_loci
 ...
 Usage: $self->associated_loci
 Desc:  find the loci associated with the feature 
...
sub associated_loci {
    ...
    my $query = $self->get_dbh()->prepare("SELECT locus_id FROM phenome.locus_dbxref
                                           ...
                                           WHERE feature.name= ? AND locus_dbxref.obsolete='f'");
...
=head2 get_accession_by_fearure_id
...
sub get_accession_by_feature_id {
    ...
    my $feature_id=shift;
    my $query = "SELECT accession FROM public.dbxref JOIN public.feature USING (dbxref_id) where (feature.name= ?) OR (feature.feature_id = ?)";
    my $sth = $self->get_dbh()->prepare($query);
    
    $sth->execute($feature_name, $feature_id);
