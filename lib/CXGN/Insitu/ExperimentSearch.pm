...
#
__PACKAGE__->uses_joinpath("probepath", [ "$s.probe", "$s.probe.probe_id = $s.experiment.probe_id"]);
__PACKAGE__->uses_joinpath("imagepath", [ "$s.image", "$s.image.experiment_id=$s.experiment.experiment_id"]);
__PACKAGE__->uses_joinpath("peoplepath", [ "$p.sp_person", "$p.sp_person.sp_person_id=$s.experiment.user_id"]);
__PACKAGE__->uses_joinpath("organism_joinpath", [ "$s.is_organism", "$s.experiment.is_organism_id=$s.is_organism.is_organism_id" ] );

# add the parameter to field mappings
#
__PACKAGE__->has_parameter(name    => "experiment_name",
                           columns => "$s.experiment.name",
			   );
__PACKAGE__->has_parameter(name    => "exp_tissue",
			   columns => "$s.experiment.tissue",
			   );
__PACKAGE__->has_parameter(name    => "exp_stage",
			   columns  => "$s.experiment.stage",
			   );
__PACKAGE__->has_parameter(name    => "exp_description",
			   columns  => "$s.experiment.description",
			   );
__PACKAGE__->has_parameter(name    => "probe_name",
			   columns  => "$s.probe.name",
			   );
__PACKAGE__->has_parameter(name    => "probe_identifier",
			   columns  => "$s.probe.identifier",
			   );
__PACKAGE__->has_parameter(name    => "image_name",
			   columns  => "$s.image.name",
			   );
__PACKAGE__->has_parameter(name    => "image_description",
			   columns  => "$s.image.description",
			   );
__PACKAGE__->has_parameter(name    => "person_last_name",
			   columns  => "$p.sp_person.last_name",
			   );
__PACKAGE__->has_parameter(name    => "person_first_name",
			   columns  => "$p.sp_person.first_name",
			   );
__PACKAGE__->has_parameter(name    => "organism_name",
...
sub request_to_params { 
	...
	if (defined($params{experiment_name})) { $self->experiment_name("ILIKE ?", "%$params{experiment_name}%"); }
	if (defined($params{exp_tissue})) { $self->exp_tissue("ILIKE ?", "%$params{exp_tissue}%"); }
	if (defined($params{exp_stage})) { $self->exp_stage("ILIKE ?", "%$params{exp_stage}%"); }
	if (defined($params{exp_description})) { $self->exp_description("ILIKE ?", "%$params{exp_description}%"); }
	if (defined($params{probe_name})) { $self->probe_name("ILIKE ?", "%$params{probe_name}%"); }
	if (defined($params{probe_description})) { $self->probe_description("ILIKE ?", "%$params{probe_description}%"); }
	if (defined($params{probe_identifier})) { $self->probe_identifier("ILIKE ?", "%$params{probe_identifier}%"); }
	if (defined($params{image_name})) { $self->image_name("ILIKE ?", "%$params{image_name}%"); }
	if (defined($params{image_description})) { $self->image_description("ILIKE ?", "%$params{image_description}%"); }
	if (defined($params{person_first_name})) { $self->person_first_name("ILIKE ?", "%$params{person_first_name}%"); }
	if (defined($params{person_last_name})) { $self->person_last_name("ILIKE ?", "%$params{person_last_name}%"); }
	if (defined($params{common_name})) { $self->common_name("ILIKE ?", "%$params{common_name}%"); }
	if (defined($params{organism_name})) { $self->organism_name("ILIKE ?", "%$params{organism_name}%"); }
...
sub _to_scalars { 
    ...
    my @keys = ( "experiment_name", "exp_stage", "exp_tissue", "exp_description", "probe_name", "probe_description", "probe_identifier", "image_name", "image_description", "person_first_name", "person_last_name", "common_name", "organism_name");
    foreach my $k (@keys) { 
...
sub to_html { 
    ...
    my @keys = ( "experiment_name", "exp_stage", "exp_tissue", "exp_description", "probe_name", "probe_description", "probe_identifier", "image_name", "image_description", "person_first_name", "person_last_name", "organism_name", "common_name");
