...
sub get_standard_species {
    ...
    if ( defined $tree ) {
        ...
        if ( defined $species_standardizer and defined( $species_standardizer->get_standard_name($species) ) ) {
            ...
            $species = CXGN::Phylo::Species_name_map->to_standard_format( $species, 1 )
...
sub make_newick_attributes {
    ...
    foreach my $attr ( $self->get_tree()->newick_shown_attributes() ) {
        ...
        if ( $attr eq "species" ) {    # species shown for leaves, or all nodes if $show_all
            ...
            $string .= "$attr=$value;"
              unless ( $self->is_leaf() && ( $attr eq "speciation" ) );    # don't show speciation attr for leaves
        }
        ...
        foreach my $n (@sub_nodes) {
            if ( $n->is_leaf() ) {
                $self->set_species( $n->get_species() )
                  if ( $self->get_species() eq "" );    # get species name from any leaf (all have same species)
            }
...
#			print("self name: ", $self->get_name(), " key: ", $self->get_node_key(), "  species: ", $self->get_species(), "\n");
     ...
     #		print("sib name: ", $sib->get_name(), " key: ", $sib->get_node_key(), "  species: ", $sib->get_species(), "\n");
                     ...
                     && ( $sib->get_node_key() < $self->get_node_key() ) )
                {
                    ...
                    $self->set_name( $self->get_name() . "|" . $sib->get_name() )
    ...
    if ( defined $root_species_hash_ref ) {
        foreach ( keys %$root_species_hash_ref ) {
            my $this_species_leaf_count =
              ( defined $species_hash_ref->{$_} ) ? $species_hash_ref->{$_} : 0;    # leaves in subtree w THIS species
            my $this_species_comp_leaf_count =
              $root_species_hash_ref->{$_} - $this_species_leaf_count;    # leaves in rest of tree w THIS species
...
sub get_max_leaf_leaf_pathlength_in_subtree_thru_node {
    my $self = shift;
    return $self->get_attribute("dist_to_leaf_longest") + $self->get_attribute("dist_to_leaf_next_longest");
}
...
sub recursive_quasiRF_distance {
    ...
    foreach my $c ( $self->get_children() ) {
        $qRFdist +=
          $c->recursive_quasiRF_distance() + ( $c->get_attribute("subtree_leaves_match") eq "true" )
          ? 0.0
          : $c->get_branch_length();
...
## should also check that
    ...
    if ( $root1->get_name() ne $root2->get_name() ) {
        print( "In robinson_foulds_distance. root implicit names are different: ",
...
#print "in delete self. key, is_root, parent: ", $self->get_node_key(), "  ", $self->is_root(), "  ", $self->get_parent(), '   ', $self->get_name(), "\n";
    if ( $self->is_root() ) {
        warn "CXGN::Phylo::Node::delete_self: You are attempting to delete the root node of the tree. key: ",
          $self->get_node_key(), "\n";
        return 1;
    }
...
sub recursive_divide_subtree_into_small_trees {
    ...
    if ( $self->get_attribute("leaf_count") <= $max_leaves ) {
        push @$small_trees_array,
          $self->copy_subtree();    #store a tree which is a copy of the subtree with root at this node
    } else {
        my @children = $self->get_children();
        foreach (@children) {
...
sub recursive_set_levels_above_distinct_species_subtree {
         ...
         && ( $self->get_attribute("leaf_count") > 1 ) )
    {
        ...
        $levels_above_distinct_species_subtree =
          ( $min_child_levels_above == $big_levels ) ? $big_levels : $min_child_levels_above + 1;
    }
...
sub speciation_at_this_node {
    ...
    if ( $nchild == 2 ) {             # bifurcation.
        ...
        return $spec_tree->get_root()->recursive_compare_species_split( $bp1, $bp2 )
...
sub recursive_compare_species_split {
    ...
    ;                    # one subtree has no species found in species tree
    my @children = $self->get_children();
        ...
        if ( $parent->get_attribute("speciation") ) {
            ...
            foreach ( $parent->get_children() ) {    # all children
                ...
                @ortholog_array = ( @ortholog_array, @imp_names )
                  if ( $self->get_attribute("species_bit_pattern") > 0 );
            }
        }
