...
=head1 DESCRIPTION
 ...
 This script check 70 variables to test the right operation of the 
...
use Data::Dumper;
use Test::More tests => 75; # qw | no_plan |; # while developing the test
use Test::Exception;
...
my %test_values_for_empty_object=( experiment_id          => $last_experiment_id+1,
...
throws_ok { CXGN::GEM::Experiment->new($schema, 'no integer')} qr/DATA TYPE ERROR/, 
    'TESTING DIE ERROR when a non integer is used to create a protocol object with new() function';

throws_ok { CXGN::GEM::Experiment->new($schema)->set_geexperiment_row() } qr/PARAMETER ERROR: None geexperiment_row/, 
    'TESTING DIE ERROR when none schema is supplied to set_geexperiment_row() function';

throws_ok { CXGN::GEM::Experiment->new($schema)->set_geexperiment_row($schema) } qr/SET ARGUMENT ERROR:/, 
    'TESTING DIE ERROR when argument supplied to set_geexperiment_row() is not a CXGN::GEM::Schema::GeExperiment row object';

throws_ok { CXGN::GEM::Experiment->new($schema)->force_set_experiment_id() } qr/PARAMETER ERROR: None/, 
    'TESTING DIE ERROR when none experimental_design_id is supplied to set_force_experiment_id() function';

throws_ok { CXGN::GEM::Experiment->new($schema)->force_set_experiment_id('non integer') } qr/DATA TYPE ERROR:/, 
    'TESTING DIE ERROR when argument supplied to set_force_experiment_id() is not an integer';

throws_ok { CXGN::GEM::Experiment->new($schema)->set_experiment_name() } qr/PARAMETER ERROR: None data/, 
    'TESTING DIE ERROR when none data is supplied to set_experiment_name() function';

throws_ok { CXGN::GEM::Experiment->new($schema)->set_experimental_design_id() } qr/PARAMETER ERROR: None/, 
    'TESTING DIE ERROR when none experimental_design_id is supplied to set_experimental_design_id() function';

throws_ok { CXGN::GEM::Experiment->new($schema)->set_experimental_design_id('non integer') } qr/DATA TYPE ERROR:/, 
    'TESTING DIE ERROR when argument supplied to set_experimental_design_id() is not an integer';

throws_ok { CXGN::GEM::Experiment->new($schema)->set_replicates_nr('non integer') } qr/DATA ARGUMENT ERROR:/, 
    'TESTING DIE ERROR when argument supplied to set_replicates_nr() is not an integer';

throws_ok { CXGN::GEM::Experiment->new($schema)->set_colour_nr('non integer') } qr/DATA ARGUMENT ERROR:/, 
...
eval {
    ...
    is($exp_id1, $last_experiment_id+1, "TESTING STORE_EXPERIMENT FUNCTION, checking experiment_id")
	or diag "Looks like this failed";

    my $exp2 = CXGN::GEM::Experiment->new($schema, $exp_id1);
    is($exp2->get_experiment_name(), 'exp1', "TESTING STORE_EXPERIMENT FUNCTION, checking experiment_name")
	or diag "Looks like this failed";
    is($exp2->get_experimental_design_id(), $expdesign_id, "TESTING STORE_EXPERIMENT FUNCTION, checking experimental_design_id")
	or diag "Looks like this failed";
    is($exp2->get_replicates_nr(), 5, "TESTING STORE_EXPERIMENT FUNCTION, checking replicates_nr")
	or diag "Looks like this failed";
    is($exp2->get_colour_nr(), 2, "TESTING STORE_EXPERIMENT FUNCTION, checking colour_nr")
	or diag "Looks like this failed";
    is($exp2->get_contact_id(), $creation_user_id, "TESTING STORE_EXPERIMENT FUNCTION, checking contact_id")
	or diag "Looks like this failed";
    ...
    throws_ok { $exp2->store_experiment($schema) } qr/STORE ERROR: Metadbdata supplied/, 
    ...
    is($exp2->is_experiment_obsolete(), 0, "TESTING IS_EXPERIMENT_OBSOLETE FUNCTION, checking boolean")
	or diag "Looks like this failed";
    ...
    is($exp2->get_experiment_metadbdata()->get_metadata_id, $last_metadata_id+2, 
    ...
    is($exp2->get_experiment_id(), $last_experiment_id+1, 
	...
	or diag "Looks like this failed";
    is($exp2->get_description(), 'This is another test', 
    ...
    is($exp3->get_experiment_id(), $last_experiment_id+1, "TESTING NEW_BY_NAME, checking experiment_id")
   	or diag "Looks like this failed";
    ...
    throws_ok { $exp3->set_geexperimentdbxref_rows() } qr/FUNCTION PARAMETER ERROR:None geexperimentdbxref_row/, 
    'TESTING DIE ERROR when none data is supplied to set_geexperimentdbxref_rows() function';
    
    throws_ok { $exp3->set_geexperimentdbxref_rows('this is not an integer') } qr/SET ARGUMENT ERROR:/, 
    'TESTING DIE ERROR when data type supplied to set_geexperimentdbxref_rows() function is not an array reference';

    throws_ok { $exp3->set_geexperimentdbxref_rows([$schema, $schema]) } qr/SET ARGUMENT ERROR:/, 
    ...
    my $exp4 = CXGN::GEM::Experiment->new($schema, $exp3->get_experiment_id() );
    ...
    throws_ok { $exp3->store_dbxref_associations() } qr/STORE ERROR: None metadbdata/, 
    'TESTING DIE ERROR when none metadbdata object is supplied to store_dbxref_associations() function';
    
    throws_ok { $exp3->store_dbxref_associations($schema) } qr/STORE ERROR: Metadbdata supplied/, 
    ...
    $exp4->obsolete_dbxref_association($metadbdata, 'obsolete test for dbxref', $dbxref_id_list[1]);
    is($exp4->is_experiment_dbxref_obsolete($dbxref_id_list[1]), 1, 
       "TESTING OBSOLETE EXPERIMENT DBXREF ASSOCIATIONS, checking boolean") 
	or diag "Looks like this failed";

    my %expdbxref_md2 = $exp4->get_experiment_dbxref_metadbdata();
    is($expdbxref_md2{$dbxref_id_list[1]}->get_metadata_id, $last_metadata_id+5, 
       "TESTING OBSOLETE EXPERIMENT DBXREF FUNCTION, checking new metadata_id")
	or diag "Looks like this failed";

    $exp4->obsolete_dbxref_association($metadbdata, 'revert obsolete test for dbxref', $dbxref_id_list[1], 'REVERT');
    is($exp4->is_experiment_dbxref_obsolete($dbxref_id_list[1]), 0, 
       "TESTING OBSOLETE DBXREF ASSOCIATIONS REVERT, checking boolean") 
	or diag "Looks like this failed";

    my %expdbxref_md2o = $exp4->get_experiment_dbxref_metadbdata();
    my $expdbxref_metadata_id2 = $expdbxref_md2o{$dbxref_id_list[1]}->get_metadata_id();
    is($expdbxref_metadata_id2, $last_metadata_id+6, "TESTING OBSOLETE DBXREF FUNCTION REVERT, checking new metadata_id")
	or diag "Looks like this failed";

    ## Checking the errors for obsolete_pub_asociation (TEST 62 TO 65)
    
    throws_ok { $exp4->obsolete_dbxref_association() } qr/OBSOLETE ERROR: None metadbdata/, 
    'TESTING DIE ERROR when none metadbdata object is supplied to obsolete_dbxref_association() function';

    throws_ok { $exp4->obsolete_dbxref_association($schema) } qr/OBSOLETE ERROR: Metadbdata/, 
    'TESTING DIE ERROR when argument supplied to obsolete_dbxref_association() is not a CXGN::Metadata::Metadbdata object';
    
    throws_ok { $exp4->obsolete_dbxref_association($metadbdata) } qr/OBSOLETE ERROR: None obsolete note/, 
    'TESTING DIE ERROR when none obsolete note is supplied to obsolete_dbxref_association() function';
    
    throws_ok { $exp4->obsolete_dbxref_association($metadbdata, 'test note') } qr/OBSOLETE ERROR: None dbxref_id/, 
    ...
    is($exp5->get_experiment_id(), $last_experiment_id+2, 
    ...
    is(ref($expdesign2), 'CXGN::GEM::ExperimentalDesign', 
    ...
    my @target_names = ('target_test1', 'target_test2');
      ...
      foreach my $targ (@targets) {
          my $t = $o+1;
          is(ref($targ), 'CXGN::GEM::Target', "TESTING GET_TARGET_LIST function, testing object reference ($t)")
              or diag "Looks like this failed";
          is($targ->get_target_name(), $target_names[$o], "TESTING GET_TARGET_LIST function, testing target_names ($t)")
              or diag "Looks like this failed";
          $o++;
      }
