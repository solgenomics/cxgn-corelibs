...
foreach my $ref (@paths) {
	...
	print $path[$j]->[0]->get_db_name().":".$path[$j]->[0]->get_accession()." -- ".$path[$j]->[0]->get_cvterm_name()."\n";
    }
    print "\n";
}
...
for( my $i = 0; $i < scalar( @paths ); $i++ ) {
	...
	if( scalar( $xmlRoot->descendants() ) > 0 ) {
	    ...
	    my @arrayValues = ();
	    push( @arrayValues, 1 );
	    $termIndentHash{substr( $next->trimmed_text, 0, 10 )} = [@arrayValues];
	}
    }
}
...
my $newLineIndex = 0;                  # Remove blank lines by removing extra newlines; go through the string multiple
